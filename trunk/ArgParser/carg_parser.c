    ...
    int * const argindp )
{
    ...
    for( i = 0; options[i].code != 0; ++i )
        ...
        if( ambig && !exact )
        {
            add_error( ap, _T("选项 “") ); add_error( ap, opt );
            add_error( ap, _T("” 有歧义") );
            return 1;
        }
        ...
        {
            add_error( ap, _T("无法识别选项 “") ); add_error( ap, opt );
            add_error( ap, _T("”") );
            return 1;
        }
        ...
        if( opt[len+2] )        // `--<long_option>=<argument>' syntax
        {
            ...
            {
                add_error( ap, _T("选项 “--") ); add_error( ap, options[index].name );
                add_error( ap, _T("” 不允许存在参数") );
                return 1;
            }
            if( options[index].has_arg == ap_yes && !opt[len+3] )
            {
                add_error( ap, _T("选项 “--") ); add_error( ap, options[index].name );
                add_error( ap, _T("” 需要参数") );
                return 1;
            }
    ...
    while( cind > 0 )
    {
        ...
        if( code != 0 )
            for( i = 0; options[i].code; ++i )
                ...
                if( index < 0 )
                {
                    add_error( ap, _T("选项 “-") ); add_error( ap, code_str );
                    add_error( ap, _T("” 无效") );
                    return 1;
                }
